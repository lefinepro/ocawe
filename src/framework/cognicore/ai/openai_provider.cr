    ...
    class OpenAIProvider
      ...
      def generate_text(request : TextGenerationRequest) : TextGenerationResponse
        if ENV["COGNICORE_MOCK_LLM"]? == "1"
          text = "[mock openai] #{request.prompt}"
          return TextGenerationResponse.new(provider: "openai", model: request.model, text: text)
        end

        key = @api_key
        raise "OPENAI_API_KEY is required" unless key
        ...
        unless response.success?
          raise "openai request failed (#{response.status_code}): #{response.body}"
        end

        body = JSON.parse(response.body)
        text = extract_text(body)
        TextGenerationResponse.new(provider: "openai", model: request.model, text: text)
      end
      ...
      private def extract_text(payload : JSON::Any) : String
        ...
        raise "openai response is missing generated text"
      end
