...
describe CogniCore::CLI::Main do
  it "triggers workflow by id without action" do
    captured_url = ""
    captured_body = ""

    cli = CogniCore::CLI::Main.new(->(url : String, body : String) {
      captured_url = url
      captured_body = body
      CogniCore::CLI::Main::TriggerResponse.new(201, %({"ok":true}))
    })

    cli.run(["workflow", "solver"], program_name: "cogni")
  ...
  it "maps workflow key=value args to payload.input" do
    ...
    cli = CogniCore::CLI::Main.new(->(_url : String, body : String) {
      captured_body = body
      CogniCore::CLI::Main::TriggerResponse.new(200, %({"ok":true}))
    })

    cli.run(["workflow", "solver", "task=deploy", "force=true", "fast"], program_name: "cogni")
  ...
  it "routes agent command to agent trigger endpoint" do
    ...
    cli = CogniCore::CLI::Main.new(->(url : String, body : String) {
      captured_url = url
      CogniCore::CLI::Main::TriggerResponse.new(200, body)
    })

    cli.run(["agent", "reviewer"], program_name: "cogni")
  ...
  it "routes tool command via function trigger endpoint" do
    ...
    cli.run(["tool", "project_healthcheck"], program_name: "cogni")
  ...
  it "routes support command via skill trigger endpoint" do
    ...
    cli.run(["support", "onboarding"], program_name: "cogni")
  ...
  it "uses executable basename as workflow alias" do
    ...
    cli.run([] of String, program_name: "/usr/local/bin/cogni_example_workflow")

    captured_url.should eq("http://127.0.0.1:4111/v1/triggers/workflows/cogni_example_workflow")
  end
end
