...
describe "E2E: Voice Operations" do
  describe "voice-playground" do
    it "creates workflow with voice nodes" do
      ...
      workflow = CogniCore::Workflow.create_workflow("voice-playground", "Voice test")
      workflow
      ...
      workflow.nodes[0].kind.should eq(CogniCore::Workflow::NodeKind::Voice)
      workflow.nodes[1].kind.should eq(CogniCore::Workflow::NodeKind::Voice)
    end

    it "executes voice transcription workflow with text input" do
      workflow = CogniCore::Workflow.create_workflow("voice-transcribe-test", "Voice transcribe test")
      workflow
    ...
    it "executes voice synthesis workflow with text input" do
      workflow = CogniCore::Workflow.create_workflow("voice-synthesize-test", "Voice synthesize test")
      workflow
  ...
  describe "voice node execution" do
    it "processes voice node with text input" do
      workflow = CogniCore::Workflow.create_workflow("e2e-voice", "Voice test")
      workflow
        .voice("speak", config: {
