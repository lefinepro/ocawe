...
describe "E2E: Agent Interaction" do
  describe "agent with voice config" do
    it "passes voice configuration to agent result" do
      ...
      begin
        workflow = Ocawe::Workflow.create_workflow("e2e-agent-voice", "Agent with voice")
        workflow
          .agent(
            "voice-agent",
  ...
  describe "agent input handling" do
    it "builds user prompt from task field" do
      ...
      begin
        workflow = Ocawe::Workflow.create_workflow("e2e-agent-task", "Agent task input")
        workflow
    ...
    it "builds user prompt from input object" do
      ...
      begin
        workflow = Ocawe::Workflow.create_workflow("e2e-agent-input-obj", "Agent input object")
        workflow
  ...
  describe "multi-agent workflows" do
    it "chains multiple agents with output passing" do
      ...
      begin
        workflow = Ocawe::Workflow.create_workflow("e2e-multi-agent", "Multi-agent chain")
        workflow
