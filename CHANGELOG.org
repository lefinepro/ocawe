#+TITLE: Ocawe Changelog
#+AUTHOR: Ocawe Core Team
#+DATE: 2026-06-12
...
** Fixed
- Installed CLI commands now find the packaged =ocawecore= binary automatically,
  so =ocawe up= works after Nix and =install.sh= installs without trying to
  rebuild the runtime from source.
- =@[Container(...)]= parsing now reads =image= and =files= (previously =image= was
  never parsed and =files= was unavailable), and supports multi-line annotations.
- Restored compilation of the spec suite (=crystal spec=), which was broken by a
  missing =CawfileContainer#files= accessor.
- Replaced the deprecated =Process.fork= in =ocawe up -d= with a direct detached
  spawn; the recorded PID now points at the runtime process so =kill <pid>= works.

** Changed
- =@[Container(...)]= =mode:= is ignored; =packages= is now the only Cawfile
  switch that selects the Nix container builder.
- Added =ocawe dev= as the live-reload server command; container-backed dev runs
  mount the workflow directory at =/workspace= so runtime reloads see host edits.
- Removed container configuration details from the top-level CLI help output.

** Documentation
- Added install verification steps for Nix and =install.sh= installs.
- Documented the =@[Container(...)]= annotation (=image=/=packages=/=files=) in the
  workflow format guide and README.
