...
OCAWE_EXAMPLES="$(nix path-info github:lefinepro/ocawe#ocawe)/share/ocawe/caws"
ocawe up "$OCAWE_EXAMPLES/01-simple"
#+end_src
...
installs example Cawfiles and helper scripts under =share/ocawe=.
...
- =input_type= / =output_type= and =input_validate= / =output_validate= — explicit workflow API contract and runtime JSON validation
- =exec "...", runtime: {acp: {...}}= — run an external ACP-compatible agent
- =exec "git+https://github.com/org/repo/path", runtime: {"git+https"}= — clone or fast-forward pull a remote Cawfile reference over HTTPS, compile it into a cached single =ocawecore= binary, then run the selected workflow through that binary
