...
- =exec "git+https://github.com/org/repo/path", runtime: {"git+https"}= — clone or fast-forward pull a remote Cawfile reference into the Ocawe cache over HTTPS
- =exec "git+ssh://github.com/org/repo/path/workflow", runtime: {"git+ssh"}= — clone or fast-forward pull a remote Cawfile reference into the Ocawe cache over SSH
